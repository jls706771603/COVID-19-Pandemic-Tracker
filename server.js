...
const data =  require('C:/Users/jam1o/Desktop/School/capstone/capstone2/COVID-19-Pandemic-Tracker/my-app/src/states2.json')
...
// const downloadCSV = require('./csvfetcher');
// const urlUSA = require('./csvfetcher');
// const urlStates = require('./csvfetcher');
// const urlCounties = require('./csvfetcher');
// const dest1 = require('./csvfetcher');
// const dest2 = require('./csvfetcher');
// const dest3 = require('./csvfetcher');
// const dest4 = require('./csvfetcher');
...
// downloadCSV()
// downloadCSV(urlStates, dest2)
// downloadCSV(urlCounties, dest3)
// downloadCSV(urlCounties, dest4)
...
//fully parses objs array for state data, pushes to 'fullStates' array
  ...
  for(let i = 0; i < objs.length; i++) {
    ...
    if(coords.length === 1) {
      ...
      for(let j = 0; j < holder.length; j++) {
        ...
        newObj.id = objs[i].id
        // console.log("about to look for:" + newObj.name)
        for(let k = 0; k < stateList.length; k++) {
          if(newObj.id === stateList[k].id){
            console.log("found for : " +newObj.name)
            newObj.population = stateList[k].population
            newObj.cases = stateList[k].cases
            newObj.deaths = stateList[k].deaths
            newObj.vacRate = stateList[k].vacRate
            newObj.caseRate = ((newObj.cases)/(newObj.population)*100)
            if(newObj.caseRate === Infinity){
              newObj.caseRate = 15
            }
          }
        }
        // newObj.population = stateList.find(state => state.name === newObj.name).population
        // newObj.cases = stateList.find((e) => e.name === newObj.name).cases
        // newObj.deaths = stateList.find((e) => e.name === newObj.name).deaths
        // newObj.vacRate = stateList.find((e) => e.name === newObj.name).vacRate

        newArr.push(newObj)
      }
    } 
    //if array has nested arrays for islands
      ...
      for(let j = 0; j < coords.length; j++){
        ...
        for(let k = 0; k < coordstepone.length; k++){
          ...
          for(let l = 0; l < coordsteptwo.length; l++){
            ...
            newObj.name = objs[i].name
            newObj.id = objs[i].id
            for(let m = 0; m < stateList.length; m++) {
              if(newObj.id === stateList[m].id){
                console.log("found for : " +newObj.name)
                newObj.population = stateList[m].population
                newObj.cases = stateList[m].cases
                newObj.deaths = stateList[m].deaths
                newObj.vacRate = stateList[m].vacRate
                newObj.caseRate = ((newObj.cases)/(newObj.population)*100)
                if(newObj.caseRate === Infinity){
                  newObj.caseRate = 15
                }
            ...
            // newObj.population = stateList.find((e) => e.name = newObj.name).population
            // newObj.cases = stateList.find((e) => e.name === newObj.name).cases
            // newObj.deaths = stateList.find((e) => e.name === newObj.name).deaths
            // newObj.vacRate = stateList.find((e) => e.name === newObj.name).vacRate
            // newObj.caseRate = ((newObj.cases)/(newObj.population)*100)
...
const stateGeoRef = ref.child('stateGeo');
stateGeoRef.set({fullStates})
